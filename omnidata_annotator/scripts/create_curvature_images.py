...
ACCEPTED_CURVATURE_MODES = ["GAUSSIAN_DISPLAY", "PRINCIPAL_CURVATURES", "MEAN_CURVATURE"]
...
def main():
    ...
    model_fpath = io_utils.get_model_file(basepath, typ='RAW')
    k1_fpath = model_fpath.replace(".", "_k1.").replace("obj", "ply")
    k2_fpath = model_fpath.replace(".", "_k2.").replace("obj", "ply")
    mean_fpath = model_fpath.replace(".", "_mean.").replace("obj", "ply")
    gaussian_fpath = model_fpath.replace(".", "_gaussian.").replace("obj", "ply")

    if model_fpath.__contains__('.obj'):
        os.makedirs(os.path.join(basepath, '..', 'ply'), exist_ok=True)
        ...
        model_fpath = model_fpath.replace('.obj', '.ply')
    ...
    with Profiler("Compute Principal Curvatures", logger) as prf:
        if settings.CURVATURE_OUTPUT_MODE == "PRINCIPAL_CURVATURES":

            # Compute principal curvature K1
            print("xvfb-run -a {meshlabserver} -s {k1_script} -i {input_ply} -o {k1_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                k1_script=os.path.join(scriptpath, settings.K1_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                k1_ply=k1_fpath))

            call("xvfb-run -a -s '-screen 0 800x600x24' {meshlabserver} -s {k1_script} -i {input_ply} -o {k1_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                k1_script=os.path.join(scriptpath, settings.K1_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                k1_ply=k1_fpath), shell=True)



            # Compute principal curvature K2
            print("xvfb-run -a {meshlabserver} -s {k2_script} -i {input_ply} -o {k2_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                k2_script=os.path.join(scriptpath, settings.K2_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                k2_ply=k2_fpath))

            call("xvfb-run -a -s '-screen 0 800x600x24' {meshlabserver} -s {k2_script} -i {input_ply} -o {k2_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                k2_script=os.path.join(scriptpath, settings.K2_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                k2_ply=k2_fpath), shell=True)


            model_fpath = k1_fpath

        elif settings.CURVATURE_OUTPUT_MODE == "GAUSSIAN_DISPLAY":
            # Compute gaussian curvature
            print("xvfb-run -a {meshlabserver} -s {gaussian_script} -i {input_ply} -o {gaussian_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                gaussian_script=os.path.join(scriptpath, settings.K2_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                gaussian_ply=gaussian_fpath))
            call("xvfb-run -a -s '-screen 0 800x600x24' {meshlabserver} -s {gaussian_script} -i {input_ply} -o {gaussian_ply} -om vq".format(
                meshlabserver=settings.MESHLAB_SERVER_PATH,
                gaussian_script=os.path.join(scriptpath, settings.K2_MESHLAB_SCRIPT),
                input_ply=model_fpath,
                gaussian_ply=gaussian_fpath), shell=True)

            model_fpath = gaussian_fpath
    ...
    def recolor_model(scene):
        ...
        elif settings.CURVATURE_OUTPUT_MODE == "MEAN_CURVATURE":
            means = PlyData.read(model_fpath).elements[0]['quality']
            means = clip_curvatures(means, settings.MIN_CURVATURE_RADIUS)
            colors = map_to_color(means, min_radius=settings.MIN_CURVATURE_RADIUS, type='mean')
...
def map_to_color(k1, k2=None, min_radius=settings.MIN_CURVATURE_RADIUS, type='principal'):
    if type == 'principal':
        ...
        remapped = ((k1 * min_radius) + 1.0) / 2.0
