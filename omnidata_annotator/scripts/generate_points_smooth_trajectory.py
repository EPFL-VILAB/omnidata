...
def main():
    ...
    if settings.POINT_TYPE == 'SWEEP':
        if not os.path.isdir(os.path.join(basepath, 'pano', TASK_NAME)):
            os.makedirs(os.path.join(basepath, 'pano', TASK_NAME))
    else:
...
def create_smooth_trajectory(point_datas):
    new_point_datas = []
    for point_num, point_data in enumerate(point_datas):
        if point_data is None: continue
        # find furthest camera to the point
        point_data = sorted(point_data, key=lambda k: k['camera_distance']) 
        cur_camera = point_data.pop(-1)
        key_point_data = []
        key_times = []
        key_rots = []
        key_locs = []
        ...
        while point_data:
            key_point_data.append(cur_camera)
            cur_camera_loc = cur_camera['camera_location']
            cur_camera_rot = cur_camera['camera_rotation_final']
            key_locs.append(cur_camera_loc)
            key_rots.append(cur_camera_rot)
            key_times.append(key_time)

            # find closest camera to the current camera
            closest_camera_idx, closest_dist = None, 1e6
            for idx, point_info in enumerate(point_data):
                camera_loc = point_info['camera_location']
                dist = (Vector(cur_camera_loc) - Vector(camera_loc)).magnitude
                if dist < closest_dist:
                    closest_dist = dist
                    closest_camera_idx = idx
            
            next_camera = point_data.pop(closest_camera_idx)
            next_camera_loc = next_camera['camera_location']

            # steps between the 2 cameras = 10 * distance 
            steps = int(10 * dist)
            key_time += steps

            # interpolate camera location
            direction_vector = Vector(next_camera_loc) - Vector(cur_camera_loc)
            interp_locs += [Vector(cur_camera_loc) + t * (direction_vector/steps) for t in range(steps)]

            cur_camera = next_camera

        # append last camera info
        key_point_data.append(cur_camera)
        cur_camera_loc = cur_camera['camera_location']
        cur_camera_rot = cur_camera['camera_rotation_final']
        interp_locs.append(cur_camera_loc)
        key_locs.append(cur_camera_loc)
        key_rots.append(cur_camera_rot)
        key_times.append(key_time)

        # interpolate camera rotation
        key_rots = R.from_euler('xyz', key_rots, degrees=False)
        slerp = Slerp(key_times, key_rots)
        times = [t for t in range(key_times[-1] + 1)]
        interp_rots = slerp(times)
        interp_rots = interp_rots.as_euler('xyz', degrees=False)
        ...
        for t in times:
            if t in key_times: 
                ...
                cur_point_info_copy['camera_rotation_final'] = tuple(list(interp_rots[t]))
...
# [{'camera_uuid': '0000', 'camera_rotation_final': (1.3944408893585205, -0.06194324791431427, -2.827867031097412), 'point_normal': (0.019952937960624695, 0.0018465041648596525, 0.9997991919517517), 
# 'camera_rotation_final_quaternion': (0.1393449604511261, 0.07681077718734741, -0.6376103758811951, -0.7537480592727661), 'skybox_img': 1, 'field_of_view_rads': 2.0944727470871287, 
# 'camera_rotation_original': (1.5707963705062866, -0.09682545810937881, -1.9496952295303345), 'obliqueness_angle': 1.399002194404602, 'camera_distance': 7.451078625816716, 'resolution': 512, 
# 'point_uuid': '0', 'camera_location': (2.9754741191864014, -1.0982224941253662, -0.28674250841140747), 'skybox_img_path': './img/high/0000_skybox1.jpg', 
# 'point_location': (5.16227388381958, -8.100662231445312, -1.5914720296859741), 'point_pitch': -0.2508219917628427, 
# 'camera_rotation_from_original_to_final': (0.09703586995601654, 0.15091194212436676, -0.8848536610603333)}, 

# {'camera_uuid': '0001', 'camera_rotation_final': (1.0271700620651245, -0.003476901911199093, -2.615150213241577), 'point_normal': (0.019952937960624695, 0.0018465041648596525, 0.9997991919517517), 
# 'camera_rotation_final_quaternion': (0.22744880616664886, 0.12637096643447876, -0.47477415204048157, -0.840765655040741), 'skybox_img': 1, 'field_of_view_rads': 1.8203398473733197, 
# 'camera_rotation_original': (1.5707963705062866, 0.13522720336914062, -1.018794298171997), 'obliqueness_angle': 1.0356643199920654, 'camera_distance': 0.9959627476733629, 'resolution': 512, 
# 'point_uuid': '0', 'camera_location': (4.735533237457275, -7.362791538238525, -1.0763205289840698), 'skybox_img_path': './img/high/0001_skybox1.jpg', 
# 'point_location': (5.16227388381958, -8.100662231445312, -1.5914720296859741), 'point_pitch': -0.408443613848414, 
# 'camera_rotation_from_original_to_final': (-0.41400331258773804, 0.3877502381801605, -1.6419984102249146)}, 

# {'camera_uuid': '0002', 'camera_rotation_final': (1.3147053718566895, -0.16488341987133026, -2.5828423500061035), 'point_normal': (0.019952937960624695, 0.0018465041648596525, 0.9997991919517517), 
# 'camera_rotation_final_quaternion': (0.2659156620502472, 0.10526000708341599, -0.6033138036727905, -0.7444606423377991), 'skybox_img': 4, 'field_of_view_rads': 1.0585489389926874, 
# 'camera_rotation_original': (1.5707963705062866, -0.16527502238750458, -2.652320623397827), 'obliqueness_angle': 1.3265330791473389, 'camera_distance': 3.038203571608624, 'resolution': 512, 
