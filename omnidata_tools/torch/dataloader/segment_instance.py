...
from .task_configs import OmnidataSegm

# class OmnidataSegm:
#     @classmethod
#     def encode(cls, img):
#         ''' 
#           Takes a tensor [[B], C, W, H].
#           Omnidata encodes class + instance info into a png:
#             R: Class ID
#             2**8 * G + B: Instance ID (per class)
#         '''
#         if img.ndim == 4: img = img.permute([1,0,2,3])
#         R, G, B = img.int()
#         return R * 2**16 + G * 2**8 + B
    
#     @classmethod
#     def get_class(cls, img): return (img / 2**16).floor().int()

#     @classmethod
#     def get_class_instance(cls, img): return (img % 2**16)
...
def extract_instances(img, return_masks=False):    
    ...
    # class_map, G, B = img
    # instance_map = G*2**8 + B
    # instance_map = class_map*2**16 + G*2**8 + B
    instance_map = OmnidataSegm.get_class_instance(img)
    class_map    = OmnidataSegm.get_class(img)
    ...
    for class_idx in all_class_idxs:
        ...
        for instance_idx in class_instance_idxs:
            ...
            labels.append(2**16*class_idx + instance_idx)
...
def plot_instances(
    ...
    instance_map = copy.deepcopy(img)
    instance_ids = torch.unique(instance_map).tolist()
    colors       = torch.tensor(random_colors(len(instance_ids)))
    colormapped  = torch.zeros(list(instance_map.shape) + [3])
    for instance_id, color in zip(instance_ids, colors):
        colormapped[instance_map == instance_id] = color
    
    color_viz     = Image.fromarray((255*colormapped).byte().cpu().numpy())
    width, height = color_viz.size
    color_viz     = color_viz.resize((width*plot_scale_factor, height*plot_scale_factor))
    draw          = ImageDraw.Draw(color_viz)
    
    boxes, labels, masks = extract_instances(img)
    for box, label, color in zip(boxes, labels, colors):
        c = tuple([int(c*255) for c in color])
        draw.rectangle(list(box*plot_scale_factor), outline=c)
        clz, name = OmnidataSegm.get_class(label), OmnidataSegm.get_class_instance(label)
        draw.text(list(box[[0,1]]*plot_scale_factor), f'{clz}_{name}', fill='black')
    
    if return_PIL: return color_viz
    else:          return color_viz / 255.0
