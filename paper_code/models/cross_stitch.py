...
from multi_task_model import get_backbone, get_head, MultiTaskModel
...
class CrossStitchNetwork(nn.Module):
    ...
    def __init__(self, tasks, backbone, head, \
        stages: list, channels: dict, alpha: float, beta: float, pretrained, dilated):
        ...
        for task in tasks:
            ...
            decoder_dict[task] = model.module.decoders[task]

        # Tasks, backbone and heads
        self.tasks = tasks
        self.backbone = torch.nn.ModuleDict(backbone_dict)
        self.heads = torch.nn.ModuleDict(decoder_dict)
        self.stages = stages
